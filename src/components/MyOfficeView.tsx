...
import { Plus, Edit, Trash2, Calendar, CheckSquare, FileText, Link, Briefcase } from 'lucide-react';
import { Project } from '../types';
import ProjectModal from './ProjectModal';

interface MyOfficeViewProps {
  projects: Project[];
  projectActions: any;
  onProjectSelect: (project: Project) => void;
}

const MyOfficeView: React.FC<MyOfficeViewProps> = ({
  projects,
  projectActions,
  onProjectSelect
}) => {
  ...
  return (
    <div className="p-4 sm:p-8 bg-gradient-to-br from-gray-50 to-gray-100/50 min-h-screen relative overflow-hidden pb-20 sm:pb-8">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-20 left-20 w-64 h-64 bg-gradient-to-br from-blue-400/10 to-teal-400/10 rounded-full blur-3xl"></div>
      </div>
      
      <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-6 sm:mb-8 space-y-4 lg:space-y-0">
        <div>
          <h1 className="text-2xl sm:text-4xl font-bold bg-gradient-to-r from-purple-900 to-blue-700 bg-clip-text text-transparent">My Office</h1>
          <p className="text-gray-600 mt-1 sm:mt-2 text-base sm:text-lg">Personal projects and private workspace</p>
        </div>
      ...
      <button
        onClick={handleCreateProject}
        className="fixed bottom-6 right-6 sm:hidden w-14 h-14 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white rounded-full flex items-center justify-center shadow-lg shadow-purple-500/25 hover:shadow-xl transform hover:scale-110 transition-all duration-300 z-30 touch-manipulation"
      >
      ...
      {filteredProjects.length === 0 ? (
        <div className="text-center py-12 sm:py-20">
          <div className="w-16 h-16 sm:w-24 sm:h-24 bg-gradient-to-br from-purple-100 to-blue-200 rounded-2xl sm:rounded-3xl flex items-center justify-center mx-auto mb-4 sm:mb-6 shadow-lg">
            <Briefcase className="w-8 h-8 sm:w-12 sm:h-12 text-purple-400" />
          </div>
          <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2 sm:mb-3">
            {statusFilter === 'all' ? 'No personal projects yet' : `No ${statusFilter} projects`}
          </h3>
          <p className="text-gray-600 mb-6 sm:mb-8 text-base sm:text-lg px-4">
            {statusFilter === 'all' ? 'Create your first personal project to get started' : `No projects with ${statusFilter} status found`}
          </p>
          <button
            onClick={handleCreateProject}
            className="w-full max-w-xs sm:w-auto bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-semibold py-3 sm:py-4 px-6 sm:px-8 rounded-xl sm:rounded-2xl transition-all duration-300 shadow-lg shadow-purple-500/25 hover:shadow-xl transform hover:scale-105 text-base"
          >
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="font-bold text-lg text-gray-900 truncate group-hover:text-purple-600 transition-colors duration-300">
                        {project.title}
                      </h3>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      ...
                      className="p-2 text-gray-500 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition-all duration-300 shadow-sm hover:shadow-md"
                    >
